...
function adminNotificationHtml(data: ConsultationEmailData): string {
  const rows = [
    ...
    .map(
      ([label, value]) =>
        `<tr><td style="padding:8px 12px;font-weight:600;background:#f5f5f5;border:1px solid #e0e0e0;white-space:nowrap">${label}</td><td style="padding:8px 12px;border:1px solid #e0e0e0">${value}</td></tr>`
    )
    .join('')
...
function applicantConfirmationHtml(data: ConsultationEmailData): string {
  return `
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
      ...
      <p style="color:#374151">
        문의사항은 연락처 <strong>admin@paulacademy.net</strong>으로 보내주세요.
      </p>
      <p style="color:#6b7280;font-size:12px;margin-top:24px">
        본 이메일은 발신 전용입니다. ${SITE_NAME}
      </p>
    </div>
  `
}
